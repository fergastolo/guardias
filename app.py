...
if st.button("CALCULAR PLANIFICACIÓN", type="primary", use_container_width=True):
    df_f = resolver()
    if df_f is not None:
        def build_stats(df, col, staff_in, es_res):
            if staff_in.empty: return None
            staff = staff_in.copy()
            # Ordenamos R4 -> R1, y luego alfabéticamente si hay empate
            if es_res and "R" in staff.columns:
                staff = staff.sort_values(by=["R", "Nombre"], ascending=[False, True])
        ...
        def build_theoretical(staff_in, es_res):
            if staff_in.empty: return None
            staff = staff_in.copy()
            # Ordenamos R4 -> R1, y luego alfabéticamente
            if es_res and "R" in staff.columns:
                staff = staff.sort_values(by=["R", "Nombre"], ascending=[False, True])
