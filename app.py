            ...
            for d in range(n_dias):
                ...
                if fijo.get(rol) == nom:
                    ...
                    if rango_fechas[d] in st.session_state.ausencias_globales.get(nom, set()):
                        ...
                        errores.append(f"**{nom}** tiene la guardia del {f_str} pero está **AUSENTE**.\n💡 Sugerencias: {', '.join(sug) if sug else 'Nadie disponible'}")

            for i in dias_idx:
                if i + 1 in dias_idx:
                    sug1 = obtener_disponibles(i, df_staff, is_adj, nom)
                    errores.append(f"**{nom}** tiene guardias consecutivas: {rango_fechas[i].strftime('%d/%m')} y {rango_fechas[i+1].strftime('%d/%m')}.")
    ...
    def aplicar(staff, prefix, es_adj):
        ...
        for d in range(n_dias):
            ...
            for r in range(n):
                ...
                if not es_adj and rango_fechas[d].weekday() == 4 and d+2 < n_dias: model.Add(v[(r, d)] <= v[(r, d+2)])
